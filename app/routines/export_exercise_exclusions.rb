class ExportExerciseExclusions
  ...
  def excluded_exercises
    @excluded_exercises ||= CourseContent::Models::ExcludedExercise.preload(
                              course: [:profile, { teachers: { role: { role_user: :profile } } }]
                            ).sort_by(&:exercise_number)
  end

  def exercises_hash_by_page_uuid
    @exercises_hash_by_page_uuid ||= Content::Models::Exercise
                                       .where(number: all_excluded_exercise_numbers)
  ...
  def ee_by_course
    excluded_exercises.group_by(&:course).map do |course, excluded_exercises|
      ...
      Hashie::Mash.new(
        ...
        excluded_exercises_numbers_with_urls: get_ee_numbers_with_urls_by_ee(numbers),
        page_uuids_with_urls: get_page_uuids_and_urls_by_ee_numbers(numbers)
      )
    end
  end
